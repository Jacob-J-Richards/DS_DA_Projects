...
From all of these plots it's evident that there was no indication that failure rate of the observations with the combination of variables in which the anomaly occurred was any different from the rest of the data. Something happened which caused the payment failure rate to spike as it did.

If the anomaly could not have been predicted, when is the soonest we could had known it was happening so we could start implementing damage control.

This is a EWMA control chart comparing the anomaly data to the normal data without accounting for the difference of sample sizes.

The anomaly data breaches control limits indicating that something is wrong before the anomaly event occurs.

nah we need to make a vector which collects the number of samples of each hour of which each hour failure rate was produced

However, if you account for sample size and produce the same chart comparing a sample of the normal data of equal size to the anomaly data, the normal data also breaches normal limits.

```{r}

sample_sizes <- data.frame(transactions_subset[,2])

colnames(sample_sizes) <- c("transactions_subset")

failure_rates <- data.frame(proportion_subset,proportion_compliment)

failure_rates_sample_size <- cbind(sample_sizes,failure_rates)

write.csv(failure_rates_sample_size,file="failure_rates_sample_size.csv")
...
# anamoly data 
data <- failure_rates_sample_size

# Parameters for EWMA
lambda <- 0.2  # Smoothing parameter
nsigmas <- 3    # Number of standard deviations for control limits

# Compute mean and standard deviation for compliment
center <- mean(data$proportion_compliment, na.rm = TRUE)
std_dev <- sd(data$proportion_compliment, na.rm = TRUE)

# Initialize EWMA values
ewma_values <- c(center)  # Start EWMA at the center (mean of compliment)
UCL <- numeric()  # Initialize an empty vector for UCL
LCL <- numeric()  # Initialize an empty vector for LCL

# Compute EWMA and dynamic control limits with sample size adjustment
for (i in seq_along(data$proportion_subset)) {
  value <- data$proportion_subset[i]
  sample_size <- data$sample_sizes[i]
  
  # Compute EWMA value
  ewma <- lambda * value + (1 - lambda) * tail(ewma_values, 1)
  ewma_values <- c(ewma_values, ewma)
  
  # Adjust control limit based on sample size
  control_limit <- (std_dev / sqrt(data$transactions_subset)) * sqrt((lambda / (2 - lambda)) * (1 - (1 - lambda)^(2 * i)))
  UCL <- c(UCL, center + nsigmas * control_limit)
  LCL <- c(LCL, center - nsigmas * control_limit)
}

# Drop the first placeholder EWMA value
ewma_values <- ewma_values[-1]

# Ensure UCL and LCL have the same length as EWMA values
UCL <- UCL[1:length(ewma_values)]
LCL <- LCL[1:length(ewma_values)]

# Create a data frame for plotting
plot_data <- data.frame(
  Index = seq_along(ewma_values),
  EWMA = ewma_values,
  UCL = UCL,
  LCL = LCL,
  Center = rep(center, length(ewma_values))
)
...
ggplot(plot_data, aes(x = Index)) +
  geom_line(aes(y = EWMA, color = "EWMA"), size = 1) +
  geom_line(aes(y = UCL, color = "UCL (Upper Control Limit)"), linetype = "dashed") +
  geom_line(aes(y = LCL, color = "LCL (Lower Control Limit)"), linetype = "dashed") +
  geom_hline(aes(yintercept = Center, color = "Center Line (Mean)"), linetype = "solid") +
  labs(title = "EWMA Chart", x = "Sample Index", y = "EWMA Value") +
  theme_minimal() +
  scale_color_manual(
    name = "Legend",
    values = c("EWMA" = "blue", "UCL (Upper Control Limit)" = "red",
               "LCL (Lower Control Limit)" = "green", "Center Line (Mean)" = "black")
  ) +
...
what if you plotted transaction failure rates as a function of volume

what if this anamoly occured because there was increidbly high transaction
