...
top_quartile <- quantile(data$weighted, 0.9997)
...
weighted_fr_sum_per_hour <- aggregate(data$weighted,by=list(data$hr),sum)
plot(x=seq(1,72,1),y=weighted_fr_sum_per_hour[,2],type="o")

unique(data$pmt)
write.csv(file="by_dimension.csv",data)
```
