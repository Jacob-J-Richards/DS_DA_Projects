...
```{r}
paytm_subset <- data[(data[,5] %in% c("PAYTM", "PAYTM_V2", "PAYTM_UPI","notprovided")) & (data[,4] == "UPI"),]

paytm_compliment <- data[data[,5] != "PAYTM" | data[,5] != "PAYTM_V2" | data[,5] != "PAYTM_UPI",]
```
plot % failure by hour
```{r}
paytm_subset
unique_hours <- unique(paytm_subset$hr)
...
proportion
plot(x=seq(1,72,1),y=proportion,type="o")
```

```{r}
unique_hours <- unique(paytm_compliment$hr)
...
proportion_c
plot(x=seq(1,72,1),y=proportion_c,type="o")
```
