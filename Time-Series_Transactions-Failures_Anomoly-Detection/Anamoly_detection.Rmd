...
sampled_rows <- paytm_compliment[sample(nrow(paytm_compliment), 500, replace = TRUE), ]

uniform_values <- runif(500, min = 0, max = 1)

sampled_rows <- sampled_rows[, !(names(sampled_rows) %in% "uniform_random")]





paytm_compliment <- sampled_rows
...
```{r}
ggplot(data=try,aes(proportion_c),color="blue") + geom_density() + labs(title="distribution of normal entire span") +
geom_density(data=try,aes(proportion_subset),color="red")



compliment_before_shift <- proportion_c[1:40]
write.csv(file="compliment_before_shift.csv",compliment_before_shift)
subset_before_shift <- proportion_subset[1:40]
write.csv(file="subset_before_shift.csv",subset_before_shift)
subset_during_shift <- proportion_subset[42:57]
subset_zero_to_sixty <- proportion_subset[1:60]


before_shift <- as.data.frame(cbind(compliment_before_shift,subset_before_shift,subset_during_shift,subset_zero_to_sixty))

#ggplot(data=before_shift,aes(compliment_before_shift)) + geom_density() + labs(title="distribution of normal before shift")
#ggplot(data=before_shift,aes(subset_before_shift)) + geom_density()  + labs(title="distribution of anamoly before shift")
...
```{r}
ggplot(data=try,aes(proportion_subset)) + geom_density(fill="blue") + labs(title="distribution of anamoly entire span")
ggplot(data=before_shift,aes(subset_before_shift)) + geom_density(fill="blue")  + labs(title="distribution of anamoly before shift")
ggplot(data=before_shift,aes(compliment_before_shift)) + geom_density(fill="red") + labs(title="distribution of normal before shift")
ggplot(data=before_shift,aes(subset_during_shift)) + geom_density(fill="blue")  + labs(title="distribution of anamoly during shift")
ggplot(data=before_shift,aes(subset_zero_to_sixty)) + geom_density(fill="blue")  + labs(title="Hours before ")
```


No you couldn't had predicted the anamoly, the mean clealry shifted and re-normalized. If you take 500 random samples from the rest of the data set and compair the curve between that and the subset to adjsut for sample size, the 500 sample from the non anamoly data set will actually be worse. 
