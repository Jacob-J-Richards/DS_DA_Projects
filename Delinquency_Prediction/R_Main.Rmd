...
plot_ly(
  roc_data,
  x = ~FPR, y = ~TPR, z = ~threshold,
  type = 'scatter3d',
  mode = 'lines',
  line = list(color = ~threshold, colorscale = 'Rainbow')
) %>%
  layout(
    scene = list(
      xaxis = list(title = "False Positive Rate (1 - Specificity)"),
      yaxis = list(title = "True Positive Rate (Sensitivity)"),
      zaxis = list(title = "Decision Threshold")
    ),
    title = "3D ROC Curve for Multinomial Logistic Regression"
  )
```
